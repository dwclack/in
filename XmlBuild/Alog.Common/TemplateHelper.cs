...
namespace Alog.Common
{
    public class TemplateHelper
    {
        ...
        public void ThreadHandle(object ClsParam)
        {
            ...
            lock (padlock)
            {
                AllOK[cls.RepTitle] = false; //关闭，在线程结束时再打开
            }
            ...
            try
            {
                ...
                foreach (DataRow dr in dtMaster.Rows)   //对主表数据逐条处理，每条生成一个报文
                {
                    try
                    {
                        ...
                        ClsLog.AppendDbLog( cls.dr["RepTitle"].ToString() + ":" + xmlFile, cls.dr["RepTitle"].ToString(), ErrorLevel.Info, ClsLog.ProjectName);
                        Flag = 9;
                    ...
                    catch (Exception ex)
                    {
                        ...
                        ClsLog.AppendDbLog( ex.Message + "_线程结束", cls.dr["RepTitle"].ToString(), ErrorLevel.Error, ClsLog.ProjectName);
            ...
            catch (Exception ex)
            {
                ...
                ClsLog.AppendDbLog(" Flag=" + Flag.ToString() + ex.Message, cls.dr["RepTitle"].ToString(), ErrorLevel.Error, ClsLog.ProjectName);
        ...
        private static DataTable _OAuth_App_Cmd;
        public static DataTable OAuth_App_Cmd
        {
            get
            {
                if (_OAuth_App_Cmd != null)
                {
                    return _OAuth_App_Cmd;
                }
                if (_OAuth_App_Cmd == null)
                {
                    string sql = @"SELECT b.ID,[AutoID],[TemplateName],[methodname],[TemplateXml],
[TemplateJson],[DataSourceType],[Split1],[Split2],[DataSourceSelect],
[ToDSID],[ExecSql],[parentid],[LabelCode],[MasterSQL],[QuerySQL],
[CompleteSQL],ErrorSQL,[XMLContent],[JSONContent],[Parameters],[Service],[RcoID]  
 FROM [Apo_temp] a join  [Apo_Aip2Tmp] b on a.[AutoID] = b.[ImpID] ORDER BY b.ID";
                    _OAuth_App_Cmd = DBAccess.GetDataTable(sql, 7);
                }
                return _OAuth_App_Cmd;
            }
        }

        private static DataTable _OAuth_App_Cmd1;
        public static DataTable OAuth_App_Cmd1
        {
            get
            {
                if (_OAuth_App_Cmd1 != null)
                {
                    return _OAuth_App_Cmd1;
                }
                if (_OAuth_App_Cmd1 == null)
                {
                    string sql = @"SELECT [AutoID],[TemplateName],[methodname],[TemplateXml],
[TemplateJson],[DataSourceType],[Split1],[Split2],[DataSourceSelect],
[ToDSID],[ExecSql],[parentid],[LabelCode],[MasterSQL],[QuerySQL],
[CompleteSQL],ErrorSQL,[XMLContent],[JSONContent],[Parameters],[Service]  
 FROM [Apo_temp]";
                    _OAuth_App_Cmd1 = DBAccess.GetDataTable(sql, 7);
                }
                return _OAuth_App_Cmd1;
            }
        }

        private static DataTable _Api_Spc_Cmd;

        public static DataTable Api_Spc_Cmd
        {
            get
            {
                if (_Api_Spc_Cmd != null)
                {
                    return _Api_Spc_Cmd;
                }
                else
                {
                    string sql = @"select s.id stid, tcid, tempvalue, tempremark, importname, remark
                             from Apo_specialTmp s
                             inner join Apo_tempClass c
                             on s.tcid = c.id";
                    _Api_Spc_Cmd = DBAccess.GetDataTable(sql, 7);
                    return _Api_Spc_Cmd;
                }
            }
        }

        private static DataTable _Api_Used_Cmd;

        public static DataTable Api_Used_Cmd
        {
            get
            {
                if (_Api_Used_Cmd != null)
                {
                    return _Api_Used_Cmd;
                }
                else
                {
                    string sql = @"SELECT  [PostTable] ,[LogicTable] ,[TurnonField] ,[TurnonValue]
                                  FROM [kjsd_eip].[dbo].[Use2Apo]";

                    _Api_Used_Cmd = DBAccess.GetDataTable(sql, 7);
                    return _Api_Used_Cmd;
                }
            }
        }
