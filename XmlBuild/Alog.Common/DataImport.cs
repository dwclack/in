...
namespace Alog.Common
{
    public class DataImport
    {
        ...
        public void ImportFile(string datatype, string method, Dictionary<string, string> parameters, string newId, string ownerId, string warehouseId, out string resultStr, out int apiLogId)
        {
            ...
            string tempGuid = "";
            ImportFile(datatype, method, parameters, newId, ownerId, warehouseId, out  resultStr, out  apiLogId,ref tempGuid, ref  OutDs);
        }
        public void ImportFile(string datatype, string method, Dictionary<string, string> parameters, string newId, string ownerId, string warehouseId, out string resultStr, out int apiLogId, ref string tempGuid, ref DataSet OutDs)
        {
            apiLogId = 0;
            ...
            try
            {
                ...
                //string version = EncodeDatas(importDS, template);
            ...
            catch (Exception ex)
            {
                ...
                ClsLog.AppendDbLog(method + ":" + StringHelper.GetExceptionMsg(ex), "ImportFile", ErrorLevel.Error, ClsLog.ProjectName);
            }
        }


        public string PreVerify(ImportTemplateInfo template, DataSet importDS)
        {
        ...
        public string ImportToDB(Dictionary<string, string> parameters, string guid, ImportTemplateInfo template, DataSet importDS)
        {
            try
            {
                VerifyImportDatas(importDS, template);
            ...
            if (parameters != null && parameters.Count > 0)
            {
                ...
                {
                    ownerId = parameters["ownerId"].ToString();
                }
            }

            string inGuid = guid;
            return ImportDatasIntoDB(parameters, guid, ownerId, warehouseId, template, importDS, ref inGuid);
        }
        ...
        private string ImportDatasIntoDB(Dictionary<string, string> parameters, string newId, string ownerId, string warehouseId, ImportTemplateInfo template, DataSet importDS, ref string inGuid)
        {
            ...
            {
                //输入数据完执行
                string ExecSql = template.ExecSql;
                if (resultStr == string.Empty)
                 {
                    if (!string.IsNullOrEmpty(ExecSql))
                    {
                        try
                        {
                            ...
                            ExecSql = ExecSql.Replace("{#GUID}", tempGuid)
                                                     .Replace("{#WarehouseId}", warehouseId)
                                                     .Replace("{#OwnerId}", ownerId);
                            ExecSql = common.ReplaceParameter(ExecSql, parameters);
                            DBAccess.ExeuteSQL(ExecSql, dataSourceId);
                        }
                        catch (Exception ex)
                        {
        ...
        /// <summary>
        /// 模板中需要AES加密的字段对应的报文字段进行加密
        /// </summary>
        /// <param name="importDS"></param>
        /// <param name="template"></param>
        private string EncodeDatas(DataSet importDS, ImportTemplateInfo template)
        {
            string version = "";
            foreach (ImportTableInfo table in template.Tables)
            {
                var list = table.Columns.Where(o => o.Encoded == 1).ToList();
                if (list.Count == 0)
                {
                    continue;
                }

                if (importDS.Tables.Contains(table.SourceTableName) && list.Count > 0)
                {
                    DataTable importDT = importDS.Tables[table.SourceTableName];
                    string[]  dtcl= new string[importDT.Rows.Count];
                    int i = 0;
                    foreach (DataRow dr in importDT.Rows)
                    {
                        //foreach (ImportColumnInfo column in table.Columns)
                        dtcl[i] = "0";
                        foreach (ImportColumnInfo column in list)
                        {
                            if (importDT.Columns.Contains(column.SourceColumnName))// && column.Encoded == 1)
                            {
                                if (dtcl[i] == "0")
                                {
                                    dtcl[i] = "1";
                                    version = RepXml.VersionId;
                                    var data = dr[column.SourceColumnName].ToString();
                                    dr[column.SourceColumnName] =
                                        string.IsNullOrWhiteSpace(data) ? data :
                                        Aes256Util.Aes256Encode(data, RepXml.AesKey, RepXml.AesVector);
                                }
                            }
                        }
                        i++;
        ...
        public ImportTemplateInfo GetApoTemplate(string method)
        {
        ...
        public DataSet GetImportDS(string postdata, string dataFormat, Dictionary<string, string> parameters)
        {
            ...
            if (!string.IsNullOrEmpty(postdata))
            {
                ...
                else if (dataFormat.Equals("json", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    try
                    {
                        dsImportDatas = JsonConvert.DeserializeObject<DataSet>(postdata, new DataSetConverter());
        ...
        private void InsertDataSet(Dictionary<string, string> parameters, DataSet importDS, ImportTemplateInfo template, string newId, string ownerId, string warehouseId, out string resultStr, ref string inGuid)
        {
            ...
            string[] replaceStr = ClsLog.GetAppSettings("eipReplace").Split('|');
            bool replaceFlag = replaceStr.Length >= 2;
                ...
                foreach (var table in template.Tables)
                {
                    if (handleStr == string.Empty)
                    {
                        if (importDS.Tables.Contains(table.SourceTableName))
                        {
                            //XML导入数据开始
                            string tableName = table.SourceTableName;
                            table.FilterSql = replaceFlag 
                                ? table.FilterSql.Replace("~", "'").Replace(replaceStr[0], replaceStr[1])
                                :table.FilterSql.Replace("~", "'");
                            table.CheckRepeatSql = replaceFlag
                                ? table.CheckRepeatSql.Replace("~", "'").Replace(replaceStr[0], replaceStr[1])
                                : table.CheckRepeatSql.Replace("~", "'");
                            table.InsertSql = replaceFlag
                                ? table.InsertSql.Replace("~", "'").Replace(replaceStr[0], replaceStr[1])
                                : table.InsertSql.Replace("~", "'");
                            table.BulkInsertSql = replaceFlag
                                ? table.BulkInsertSql.Replace("~", "'").Replace(replaceStr[0], replaceStr[1])
                                : table.BulkInsertSql.Replace("~", "'");
                            ...
                            string version = EncodeDatas(importDS, template);
                            table.InsertSql = replaceFlag
                                ? table.InsertSql.Replace("{$EncodedVersion$}", version).Replace(replaceStr[0], replaceStr[1])
                                : table.InsertSql.Replace("{$EncodedVersion$}", version);
                            table.BulkInsertSql = replaceFlag
                                ? table.BulkInsertSql.Replace("{$EncodedVersion$}", version).Replace(replaceStr[0], replaceStr[1])
                                : table.BulkInsertSql.Replace("{$EncodedVersion$}", version);//批量插入语句

                            table.InsertSql = table.InsertSql.Replace("{#GUID}", inGuid)
                         .Replace("{#WarehouseId}", warehouseId)
                         .Replace("{#OwnerId}", ownerId);
                            table.BulkInsertSql = table.BulkInsertSql.Replace("{#GUID}", inGuid)
.Replace("{#WarehouseId}", warehouseId)
.Replace("{#OwnerId}", ownerId);
                            string tempGuid = inGuid;
                            if (!string.IsNullOrEmpty(table.BulkInsertSql))
                            {
                                InsertTable(currentTable, newId, table.FilterSql, table.CheckRepeatSql, table.BulkInsertSql, template.DataSourceId, table.TargetTableKey, out handleStr);
                            }
                            else
                            {
        ...
        private void GetApoColumnsInfo(ImportTableInfo table)
        {
            ...
            foreach (DataRow dr in ys_drs)
            {
                ImportColumnInfo column = new ImportColumnInfo()
                {
                    ...
                    Encoded = GetDBIntValue(dr["Encoded"]),
                };
        ...
        private void InsertDataTable(Dictionary<string, string> parameters, DataTable importDT, string tableKey, string guid, string ownerId, string warehouseId, ImportTableInfo table, int dataSourceId, out string resultStr, ref string inGuid)
        {
            ...
            try
            {
                ...
                table.InsertSql = common.ReplaceParameter(table.InsertSql, parameters);
                table.CheckRepeatSql = common.ReplaceParameter(table.CheckRepeatSql, parameters);
                        ...
                        if (tempInsertSql != string.Empty && !isRepeatRow)
                        {
                            ...
                            if (dtResult != null && dtResult.Rows.Count > 0)
                            {
                                ...
                                if (dtResult.Columns.Contains("InGUID"))
                                {
                                    ...
                                    resultStr += "出错了！【数据重复】" + dtResult.Rows[0]["InGUID"].ToString();
                                }
                                else
                                {
        ...
        private void ReplaceSqlWithParams(string guid, string ownerId, string warehouseId, string appKey, ImportTableInfo table, Dictionary<string, SqlParam> masterParams, string masterPreStr, Dictionary<string, SqlParam> slaveParams, string slavePreStr)
        {
            ...
            {
                masterParams[masterPreStr + "appKey"].Value = appKey;
            }
        }
        ...
        /// <param name="dt"></param>
        /// <param name="insertSql"></param>
        private void InsertTable(DataTable indt, string GUID, string SELEDT_SQL, string ISREPEAT_SQL, string insertSql, int dsId, string TARGET_TABLE_MasterKey,out string  handleStr)
        {
            handleStr = "";
            string Listrep = "";
            ...
            if (!string.IsNullOrEmpty(insertSql))
            {
                ...
                {
                    string ClientScript1 = ee.Message.ToString().Replace("'", "‘") + "。出错，SQL:" + insertSql;
                    handleStr = ClientScript1;
                    ClsLog.AppendDbLog(ClientScript1+StringHelper.GetExceptionMsg(ee), "批量插入出错", ErrorLevel.Error, ClsLog.ProjectName);
            ...
            ClsLog.AppendDbLog(r.ToString(), "批量导入数据", ErrorLevel.Warning, ClsLog.ProjectName);

            //handleStr = r.ToString();
        }
        #endregion
        #region 缓存导入设置
...
FROM    [dbo].[Apo_verify]", 7);
        ...
        protected DataTable Apo_columns
        {
            get
            {
                ...
                {
                    _Apo_columns = DBAccess.GetDataTable("SELECT [COLUMN_ID],[TABLE_ID],[COLUMN_CODE],[DATA_COLUMN],[DATA_TYPE],[DATA_Expression],[DataType],[Length], [Encoded] FROM Apo_columns", 7);
        ...
        #endregion
        #region 接口服务登陆资料缓存
        private static DataTable _OAuth_APP;
        ...
        public void ImportFile(string datatype, string method, Dictionary<string, string> parameters, string newId, out string resultStr, out string inGuid, out int apiLogId)
        {
           // string errorSql = "";
            ...
            inGuid = "";
            DataSet OutDs = new DataSet();
            ImportFile(datatype, method, parameters, newId, null, null, out  resultStr, out  apiLogId, ref inGuid, ref  OutDs);
            //inGuid = newId;
            //resultStr = string.Empty;
            //int dataSourceId = 1;
        ...
        public DataSet GetImportDS(string importDataStr, string dataFormat)
        {
            DataSet ds = null;
