...
namespace Alog_WSKJSD
{
    public class RepXml
    {
        ...
        public void ThreadHandle(object ClsParam)
        {
            ...
            //NLogger.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "  " + cls.RepTitle + "_线程启动", "报文生成日志");
            ...
            try
            {
                ...
                {
                    //NLogger.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + cls.dr["RepTitle"].ToString() + "  主表sql语句为空" + cls.dr["RepTitle"].ToString() + "_线程结束", "报文生成日志");
                    lock (padlock)
                    {
                ...
                {
                    //NLogger.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "  没有要处理的" + cls.dr["RepTitle"].ToString() + "_线程结束", "报文生成日志");
                    lock (padlock)
                    {
                        AllOK[cls.RepTitle] = true;
                    }
                ...
                //主表数据一行一个报文
                int dataCount = dtMaster.Rows.Count;
                var list = dtMaster.Rows.Cast<DataRow>().ToArray();
                int pageCount = Convert.ToInt32(ClsLog.GetAppSettings("ThreadCount"));
                int pageSize = dataCount % pageCount == 0 ? (dataCount / pageCount) : (dataCount / pageCount + 1);
                NLogger.WriteLog("生成报文分页开始," + cls.dr["RepTitle"].ToString() + " , Data Count : " + dataCount + ", Page Size:" + pageSize, "MultiPage");
                System.Threading.Tasks.Parallel.For(0, pageCount, pageIndex =>
                {
                    ...
                    foreach (DataRow dr in data)
                    {
                        ...
                        try
                        {
                            ...
                            //xmlFile = dr["MessageID"].ToString();
                            //xml模板替换主表数据
                            string ParentTempLabelHtml = cls.dr["XMLContent"].ToString();
                            ...
                            {
                                NLogger.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "  循环sql语句为空" + cls.dr["RepTitle"].ToString() + "_线程结束", "报文生成日志");
                                lock (padlock)
                                {
                                    AllOK[cls.RepTitle] = true;
                                }
                            ...
                            //xml模板替换副表数据
                            Aes256Util.DataTableAes256Decode(dt, EncodedColumns_Q);
                            ParentTempLabelHtml = ReplaceSlaveValue(ParentTempLabelHtml, dt);//替换XML模板的表体及表体明细数据
                            //如果有特殊的&lt;和&gt;字符，则替换成<和>字符，主要用于解决一些特殊的XML生成方式问题
                            ParentTempLabelHtml = ParentTempLabelHtml.Replace("&lt;", "<");
                            ParentTempLabelHtml = ParentTempLabelHtml.Replace("&gt;", ">");

                            string parentid = cls.dr["AutoId"].ToString();

                            ParentTempLabelHtml=InitLabelXML(parentid, ParentTempLabelHtml);//递归生成报文
                           ...
                           // NLogger.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "  " + cls.dr["RepTitle"].ToString() + ":" + xmlFile, cls.dr["RepTitle"].ToString());
                        ...
                        {
                            NLogger.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") +cls.dr["RepTitle"].ToString()  + ex.Message + "_线程结束", "错误日志");
                ...
                //NLogger.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "  " + cls.dr["RepTitle"].ToString() + "_线程结束", "报文生成日志");
                lock (padlock)
                {
                    AllOK[cls.RepTitle] = true;
                }
            ...
            {
                NLogger.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " Flag=" + Flag.ToString() + ex.Message, "错误日志");
        ...
        string InitLabelXML(string parentid, string ParentTempLabelHtml)
        {
            try
            {
           ;
            ...
            if (dtRepXmlSet.Select("parentid=" + parentid).Count() > 0)
            {
                ...
                for (int i = 0; i < drs.Count(); i++)
                {
                    ...
                    StempHtml = drs[i]["XMLContent"].ToString();
                    string str = @"(" + LabelCode + @"(\((\w|:|,|\s|[\u4e00-\u9fa5]|((?=[\x21-\x7e]+)[^A-Za-z0-9<]))*\)))|(" + LabelCode + ")";//表达式    
                    //string str = @"(" + LabelCode + @"(\((\w|,|.)*\)))|(" + LabelCode + ")";//表达
                    if (ParentTempLabelHtml.Contains(LabelCode))
                    {
                    ...
                    ParentTempLabelHtml= InitLabelXML(parentid, ParentTempLabelHtml);
            ...
            {
                NLogger.WriteLog("InitLabelXML方法" + ex.ToString(), "错误日志");
            }
            return ParentTempLabelHtml;
        }
