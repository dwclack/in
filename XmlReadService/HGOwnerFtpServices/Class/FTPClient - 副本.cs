...
namespace DotNet.Utilities
{
    public class FTPClient
    {
        ...
        {
            ClsLog.AppendLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "开始下载", "连接日志");
            if (!bConnected)
            {
                ClsLog.AppendLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "开始连接", "连接日志");
                Connect();
                ClsLog.AppendLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "连接完成", "连接日志");
            }
            ...
            try
            {
                ...
                ClsLog.AppendLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "获取文件" + strFiles.Length, "连接日志");
                foreach (string strFile in strFiles)
                {
                    if (!strFile.Equals(""))//一般来说strFiles的最后一个元素可能是空字符串
                    {
                        ClsLog.AppendLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "开始下载文件" + strFile, "连接日志");
                        Get(strFile.Trim(), strFolder+ @"\temp", strFile.Trim());
                        ClsLog.AppendLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "下载文件完成", "连接日志");
                        ...
                        {
                            Delete(strFile.Trim());
                            ClsLog.AppendLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "删除文件成功:" + strFile, "下载日志");
                        }
                        catch (Exception ex)
                        {
                            ClsLog.AppendLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "删除文件失败：" + ex.Message + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), "下载日志");
                        }
                        scount++;
                        if (scount > 100)
                        {
            ...
            {
                ClsLog.AppendLog("下载FTP文件失败：" + ex.Message + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), "下载日志");
            }
            ...
            //catch (Exception ex)
            //{
            //    ClsLog.AppendLog("删除FTP文件失败：" + ex.Message + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), "下载日志");
            //}
        }
        ...
        public void Get(string strRemoteFileName, string strFolder, string strLocalFileName)
        {
            ...
            catch(Exception ex)
            {
                ...
                socketControl = null;
                output.Close();
                ClsLog.AppendLog("GET方法" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + ex.Message, "下载日志");
                throw ex;
            }
        }
        ...
        public void Put(string strFolder, string strFileNameMask, string FtpUpPathBak)
        {
            ...
            try
            {
                ...
                foreach (string strFile in strFiles)
                {
                    listFileList.Add(strFile);
                    Put(strFile);
                    ClsLog.AppendLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "上传FTP文件成功:" + strFile, "上传日志");
                    try
                    {
                        ...
                        ClsLog.AppendLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "删除文件成功:" + strFile, "上传日志");
                    }
                    catch (Exception ex)
                    {
                        ClsLog.AppendLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "删除文件失败：" + ex.Message + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), "上传日志");
                    }
                    scount++;
                    if (scount > 100)
                    {
            ...
            {
                ClsLog.AppendLog("上传FTP文件失败：" + ex.Message + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), "上传日志");
            }
        }
