...
namespace AlogMkFtp
{
    public partial class AlogMkFtpService : ServiceBase
    {
        private bool blnFirstExecute = false;

        public AlogMkFtpService()
        {
            InitializeComponent();
        }
        ...
        protected override void OnStart(string[] args)
        {
            ...
            catch (Exception ex)
            {
                ...
                Fun.WriteServerLogFile(DateTime.Now.ToString() + ",启动失败！" + ex.Message);
            }
        }

        protected override void OnStop()
        {
            Fun.WriteServerLogFile(DateTime.Now.ToString() + ",服务关闭！");
        }
        ...
        public void Start()
        {
            ...
            {
                timerFtpDownload.Interval = Convert.ToInt32(Utility.App.g_strFtpDownloadInterVal) * 1000;
            ...
            {
                timerFtpDownloadSecond.Interval = Convert.ToInt32(Utility.App.g_strFtpDownloadInterVal) * 1000;
                timerFtpDownloadSecond.Enabled = true;
                Utility.Fun.WriteServerLogFile(DateTime.Now.ToString() + ",服务下载报文2开始启动！");
            }

             
            timerFtpUpload.Interval = Convert.ToInt32(Utility.App.g_strFtpUploadInterVal) * 1000;
            timerFtpUpload.Enabled = true;
        ...
        private void work(HostNum Host)
        {
            ...
            try
            {
                ...
                Utility.Fun.WriteFile("下载日志.txt",mkPub.GetHost(Host) + DateTime.Now.ToString() + ",报文数量：" + FileList.Count);
                if (FileList != null && FileList.Count > 0)
                {
                    Utility.Fun.WriteFile("下载日志.txt",mkPub.GetHost(Host)+ DateTime.Now.ToString() + ",开始下载：");
        ...
        /// <param name="e"></param>
        private void timerFtpDownload_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            {
                Utility.Fun.WriteFile("下载日志.txt", Hoststring + DateTime.Now.ToString() + ",定时下载同步任务：" + ex.Message);
            }
            finally
            {
                timerFtpDownload.Enabled = true;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void timerFtpDownloadSecond_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            HostNum Host = HostNum.ReportSecond;
            string Hoststring = "";
            try
            {
                timerFtpDownloadSecond.Enabled = false;
                Hoststring = App.g_strFtpHostReportSecond;
                workSecond(Host);
            }
            catch (Exception ex)
            {
                Utility.Fun.WriteFile("下载日志.txt", DateTime.Now.ToString() + ",定时下载同步任务：" + ex.Message);
            }
            finally
            {
                timerFtpDownloadSecond.Enabled = true;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void timerFtpUpload_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            HostNum Host;
            string Hoststring = "";
            timerFtpUpload.Enabled = false;
            try
            {
                //Host = HostNum.HZ;
        ...
        /// 检查文件路径如果没有则创建
        /// </summary>
        /// <param name="Directorys">文件路径</param>
        ...
        private static bool ExistsDirectory(string Directorys, out string Err)
        {
            ...
            try
            {
                if (!System.IO.Directory.Exists(App.g_strCurrentApplicationPath + Directorys))
                    Directory.CreateDirectory(App.g_strCurrentApplicationPath + Directorys);
                return true;
            }
            catch (Exception ex)
            {
